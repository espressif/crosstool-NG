...
newlib_headers()
{
    ...
    if [ "${CT_ARCH_XTENSA}" = "y" ]; then
        ...
        CT_DoExecLog ALL cp -r "${CT_SRC_DIR}/newlib/newlib/libc/machine/xtensa/include/."   \
                               "${CT_HEADERS_DIR}"
    fi
    CT_EndStep
}
...
newlib_AUX_BUILD() {
    ...
    find ${CT_PREFIX_DIR}/${CT_TARGET}/lib \( -name libc.a -o -name libm.a -o -name libg.a \) -print0 |
